...
func (p *peer) SendElectionMessage() error {
	...
	request[0].Ip = getLocalIp()

	strip := request[0].Ip
	strID := strip[len(strip)-3:len(strip)]
	request[0].TxId,_ = strconv.ParseUint(strID,10,64)
	//request[0].TxId = 1
	request[0].MortgageAccount = 20000
	request[0].UpTime = 1513456677
	request[0].NodeId = 1
	request[0].AnnonceRate = 1000000
