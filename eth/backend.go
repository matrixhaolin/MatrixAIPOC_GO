...
const(
	masterNodeBroadcastPriod = 100
	masterNodeRefreshPriod   = 25
	electionNetEffterTime    = 3
	blockEffectDelay         = 6
)
func electionRun( ethinfo * Ethereum) {
	...
	go func() {
		for {
			...
			if blockNum%masterNodeBroadcastPriod == electionNetEffterTime {
				...
				//Start/stop miner based on the election info
				...
				if election.LocalMasterNodeInfo.IsMinner(&elector.ElectionNet[bufferInd]) {
	...
	go func() {
		futureTimer := time.NewTicker(2 * time.Minute)
